...
hold on;
for i = 1:numel(NMC_series)
    plot(integration_matrices{i}(:, 1), integration_matrices{i}(:, 2), 'o', 'MarkerSize', 3, 'MarkerFaceColor', NMC_colors(i, :), 'MarkerEdgeColor', NMC_colors(i, :));
end
hold off;
...
while true
    ...
    figure;
    plot(integration_matrices{graphIndex}(:, 1), integration_matrices{graphIndex}(:, 2), 'o', 'MarkerSize', 3, 'MarkerFaceColor', NMC_colors(graphIndex, :), 'MarkerEdgeColor', NMC_colors(graphIndex, :));
    xlabel('Momentum Transfer Å^{-1}');
    ylabel('Aggregate Intensity (arb.u.)');
    ylim([0, 250])
